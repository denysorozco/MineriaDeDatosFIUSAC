...
### Codigo de uso del algoritmo APRIORI para buscar reglas de asociacion
```{r}
...
### Codigo de uso del algoritmo FPGrowth
```{r}
reglas_fpgrowth_d <- fim4r(datos_d, method = "fpgrowth", target="rules", supp = .2, conf = .5)
reglas_fpgrowth_df <- fim4r(datos_df, method = "fpgrowth", target="rules", supp = .2, conf = .5)

reglas_fpgrowth_d_interesantes <- head(sort(reglas_fpgrowth_d, by = "lift"), 10)
plot(reglas_fpgrowth_d_interesantes, method = "graph")

reglas_fpgrowth_df_interesantes <- head(sort(reglas_fpgrowth_df, by = "lift"), 10)
plot(reglas_fpgrowth_df_interesantes, method = "graph")
```

### Codigo de uso del algoritmo KMEANS
```{r}
# Lista de defunciones
cluster_edad_causa_d <- datos_d %>%
                        select(Edadif,Caudef)

# Dataframe
frame_edad_causa_d <- as.data.frame(cluster_edad_causa_d)

# Factor
frame_edad_causa_d$Caudef <- as.factor(frame_edad_causa_d$Caudef)

# One-hot encoding

frame_one_hot_causa <- model.matrix(~ Caudef-1, data = frame_edad_causa_d)

# Uniendo
cluster_edad_causa_d <- cbind(frame_edad_causa_d["Edadif"], frame_one_hot_causa)
rm(frame_one_hot_causa)
gc()

###### APLICANDO K-Means ######
reglas_kmeans_edad_causa_d <- kmeans(cluster_edad_causa_d, centers = 4)

# Lista de defunciones fetales
cluster_edad_causa_df <- datos_df %>%
                        select(EDADM,CAUDEF)

# Dataframe
frame_edad_causa_df <- as.data.frame(cluster_edad_causa_df)

# Factor
frame_edad_causa_df$CAUDEF <- as.factor(frame_edad_causa_df$CAUDEF)

# One-hot encoding
# Primero liberamos memoria
rm(cluster_edad_causa_df)
gc()
